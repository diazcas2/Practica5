...
View(dirty_iris)
sort(dirty_iris$Sepal.Length,decreasing = FALSE) #Outliers 49 y 73
sort(dirty_iris$Sepal.Length,decreasing = TRUE) 

sort(dirty_iris$Sepal.Width,decreasing = FALSE) #-3,0,29,30
sort(dirty_iris$Sepal.Width,decreasing = TRUE)

sort(dirty_iris$Petal.Length,decreasing = FALSE) #0, 63,23,14
sort(dirty_iris$Petal.Length,decreasing = TRUE)

sort(dirty_iris$Petal.Width,decreasing = FALSE)#inf
sort(dirty_iris$Petal.Width,decreasing = TRUE)

sort(dirty_iris$Species,decreasing = FALSE)
sort(dirty_iris$Species,decreasing = TRUE)
...
is.finite(dirty_iris$Sepal.Length)
is.finite(dirty_iris$Sepal.Width)
is.finite(dirty_iris$Petal.Length)
is.finite(dirty_iris$Petal.Width)

library(dplyr)

iris_clean <- dirty_iris %>%
  mutate(
    Sepal.Length = ifelse(!is.finite(Sepal.Length) | Sepal.Length <= 0, NA, Sepal.Length),
    Sepal.Width  = ifelse(!is.finite(Sepal.Width)  | Sepal.Width  <= 0, NA, Sepal.Width),
    Petal.Length = ifelse(!is.finite(Petal.Length) | Petal.Length <= 0, NA, Petal.Length),
    Petal.Width  = ifelse(!is.finite(Petal.Width)  | Petal.Width  <= 0, NA, Petal.Width)
  )

sort(iris_clean$Sepal.Width,decreasing = FALSE) #comprobación de que quitamos el -3
...
porc_longsep<-sum(complete.cases(iris_clean$Sepal.Length))/nrow(iris_clean) *100
porc_longsep

porc_anchsep<-sum(complete.cases(iris_clean$Sepal.Width))/nrow(iris_clean) *100
porc_anchsep

porc_longpet<-sum(complete.cases(iris_clean$Petal.Length))/nrow(iris_clean) *100
porc_longpet

porc_anchpet<-sum(complete.cases(iris_clean$Petal.Width))/nrow(iris_clean) *100
porc_anchsep

clean_iris<-na.omit(iris_clean)
...
resumenclean<resumendirty #Para compararlos
```

```{r}
madm<-function(x){
  if (all(is.na(x))) {
    warning("Todos los valores son NA")
    return(NA)
  } else {
    x_no_na <- x[!is.na(x)]
    return(1.4826*mean(abs(x_no_na-mean(x_no_na))))
  }
}

madm(clean_iris$Petal.Length)
madm(dirty_iris$Petal.Length)
```
...
reglaboxplot<-function(x){
  x_s<-quantile(x, 0.75, na.rm = TRUE)
  x_i<-quantile(x, 0.25, na.rm = TRUE)
  iqr<-x_s-x_i
  inf<-x_i-1.5*iqr
  sup<-x_s+1.5*iqr
...
reglapercentil<-function(x){
  c1<-quantile(x,0.05,na.rm=TRUE)
  c95<-quantile(x,0.95,na.rm=TRUE)
...
outliersirispet<-clean_iris %>% mutate(across(Petal.Length,list(outliers_3sigma= ~reglasigma(.x),outliers_Hampel=~reglahampel(.x),outliers_boxplot=~reglaboxplot(.x),outliers_percentiles=~reglapercentil(.x))))

outliersiristotal<-clean_iris %>% mutate(across(where(is.numeric),list(outliers_3sigma= ~reglasigma(.x),outliers_Hampel=~reglahampel(.x),outliers_boxplot=~reglaboxplot(.x),outliers_percentiles=~reglapercentil(.x))))
```
...
sum(outliersirispet$Petal.Length_outliers_3sigma)
sum(outliersirispet$Petal.Length_outliers_Hampel)
sum(outliersirispet$Petal.Length_outliers_boxplot)
sum(outliersirispet$Petal.Length_outliers_percentiles)

lista_outliers <- list(
  `3sigma` = outliersirispet %>%
    filter(Petal.Length_outliers_3sigma) %>%
    arrange(Petal.Length) %>%
    pull(Petal.Length),
  
  `Hampel` = outliersirispet %>%
    filter(Petal.Length_outliers_Hampel) %>%
    arrange(Petal.Length) %>%
    pull(Petal.Length),
  
  `Boxplot` = outliersirispet %>%
    filter(Petal.Length_outliers_boxplot) %>%
    arrange(Petal.Length) %>%
    pull(Petal.Length),
  
  `Percentiles` = outliersirispet %>%
    filter(Petal.Length_outliers_percentiles) %>%
    arrange(Petal.Length) %>%
    pull(Petal.Length)
)
...
library(ggplot2)
outliersirispet<-outliersirispet %>% pivot_longer(cols=6:9,names_to ='Outliers',values_to = 'Valores')
ggplot(outliersirispet, aes(x=Outliers,y=Petal.Length,color = 
                              Outliers))+geom_point(position='jitter')
```
...
outliersirispet<-outliersirispet %>% mutate(Petal.Length1=ifelse(is.na(Petal.Length),mean(Petal.Length,na.rm=TRUE),Petal.Length))
```
...
pet2<-outliersirispet %>% group_by(Species) %>% mutate(Petal.Length2=ifelse(is.na(Petal.Length),mean(Petal.Length,na.rm=TRUE),Petal.Length))
```
...
outliersirispet2<- pet2%>% mutate(across(Petal.Length,list(outliers_3sigma= ~reglasigma(.x),outliers_Hampel=~reglahampel(.x),outliers_boxplot=~reglaboxplot(.x),outliers_percentiles=~reglapercentil(.x))))
```
...
ggplot(outliersirispet2, aes(x =Petal.Length_outliers_percentiles, y = Petal.Length2)) +
